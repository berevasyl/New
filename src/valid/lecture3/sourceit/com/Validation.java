package valid.lecture3.sourceit.com;
...
//            number += creditCardDigits[i] * mult;
//            mult *= 10;
//        }
        for(int i = 0; i < creditCardDigits.length; i++){
//          number *= 10;
//          number += creditCardDigits[i];
            number = number * 10 + creditCardDigits[i];
        }
        return number;
    }
    ...
    public static long evalRev(long[] creditCardDigits) {
        long number =0L;
//        long mult =1L;
//        for (int i = 1; i <= creditCardDigits.length; i++){
//            number += creditCardDigits[i] * mult;
//            mult *= 10;
//        }
        for(int i = creditCardDigits.length - 1; i >= 0; i--){
    ...
    public static long[] doubleSecond(long[] creditCardDigits) {
        long[] doubleSecondNum = creditCardDigits;
        for (int i = 1; i < doubleSecondNum.length; i += 2){
            doubleSecondNum[i] *= 2;
        }
        return doubleSecondNum;
    }
    ...
    public static long sumDigits(long[] listOfDigits) {
        long sumListOfDigits = 0L;
        for (int i = 0; i < listOfDigits.length; i++){
            sumListOfDigits += (listOfDigits[i] % 10);
//            System.out.println(sumListOfDigits + " " + listOfDigits[i]+ " " + (listOfDigits[i] % 10));
            sumListOfDigits += listOfDigits[i] / 10;
//            System.out.println(sumListOfDigits + " " + listOfDigits[i]+ " " + (listOfDigits[i] / 10));
        }
        return sumListOfDigits;
    }
    ...
    public static boolean isValid(long creditCard) {
        long[] arrayDigitsCreditCard = toDigitsReverse(creditCard);
        arrayDigitsCreditCard = doubleSecond(arrayDigitsCreditCard);
        long sum = sumDigits(arrayDigitsCreditCard);
        boolean validCreditCard = false;
        if (sum % 10 == 0) validCreditCard = true;
    ...
    public static int numValid(long[] creditCards) {
        ...
        for (int i = 0; i < creditCards.length; i++) {
            if (isValid(creditCards[i])) numValidCard++;
        }
        return numValidCard;
    }
    ...
    public static void main(String[] args) {
        System.out.println((char)27 + "[34mВсього карток " + creditCards.length + (char)27 + "[0m");
    for (int j = 0; j <= creditCards.length-1; j++) {
//        System.out.print("# " + creditCards[j]);
//        System.out.print("  ∟" + countDigits(creditCards[j]) + "  ← ");
        long[] a = toDigitsReverse(creditCards[j]);
        for (int i = 0; i < a.length; i++) {
//            System.out.print(a[i]);
        }

//        System.out.print(" =" + evalRev(a) + "  → ");
        long[] b = toDigits(creditCards[j]);
        for (int i = 0; i < b.length; i++) {
//            System.out.print(b[i]);
        }
//        System.out.print("  W ");
        long[] c = doubleSecond(toDigits(creditCards[j]));
        for (int i = 0; i < c.length; i++) {
//            System.out.print(c[i] + " ");
        }
        long dobleSum = sumDigits(c);
//        System.out.print("  ∑∑ " + dobleSum);
//        System.out.print("  ≡ "+ eval(b));
        long sum = sumOfNumber(creditCards[j]);
//        System.out.print("  ∑ " + sum);
        if (isValid(creditCards[j])) {
//            System.out.println((char) 27 + "[31mWarning! " + (char)27 + "[0m");
//            30 - чорний, 31 - червоний, 32 - зелений, 33 - жовтий, 34 - синій, 35 - малиновий, 36 - блакитний, 37 - білий.
        ...
        if ((j + 1) % 6 == 0) System.out.println();
            ...
            int numValidCards = numValid(creditCards);
            System.out.printf((char)27 + "[34mВалідних %3d ", numValidCards);
            switch (numValidCards % 10) {
                case 1:
                    if (numValidCards == 11) {
                        System.out.println(" карток " + (char) 27 + "[0m");
                    }
                    else System.out.println(" картка " + (char) 27 + "[0m");
                    break;
                case 2:
                case 3:
                case 4:
                    if (numValidCards > 20 || numValidCards < 10) {
                        System.out.println(" картки " + (char) 27 + "[0m");
                        break;
                    }
                default:
                    System.out.println(" карток " + (char) 27 + "[0m");
    ...
    public static void test19() {
        boolean testIsValid = isValid(5256283618614517L);
        assert testIsValid == true : "Test19 is not passed";
    }

    public static void test20() {
        boolean testIsValid = isValid(4556945538735694L);
        assert testIsValid == false : "Test20 is not passed";
    }

    public static void test21() {
        boolean testIsValid = isValid(0000000000000000L);
        assert testIsValid == true : "Test21 is not passed";
    }
